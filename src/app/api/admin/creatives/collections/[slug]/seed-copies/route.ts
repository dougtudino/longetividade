...
// Popula CreativeCopy default pra cada Creative da collection, matching
// por slug/tags. Idempotente — pula creatives que ja tem copy.
//
// Foco inicial: launch-001-pioneer. Outras collections caem em default
// generico.
...
export async function POST(
  ...
  const collection = await prisma.creativeCollection.findUnique({
    where: { slug },
    include: { creatives: { include: { copies: true } } },
  });
  if (!collection) {
    return NextResponse.json({ ok: false, error: "Collection nao encontrada" }, { status: 404 });
  }
  ...
  return NextResponse.json({ ok: true, results, count: results.filter((r) => r.created).length });
}
